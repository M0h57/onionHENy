/* Host tests for orion_ready protocol + runtime flags. */
...
static int test_ready_reject_slash(void) {
  ...
  TEST_ASSERT_TRUE(!orion_ready_signal("a/b"));
  TEST_ASSERT_TRUE(!orion_ready_signal("dot.name"));
  TEST_ASSERT_TRUE(!orion_ready_signal(""));
  TEST_ASSERT_TRUE(!orion_ready_signal(NULL));
  return 0;
}
...
static int test_ready_path_builder(void) {
  char buf[128];
  TEST_ASSERT_TRUE(orion_ready_path(ORION_FLAG_FPS_OVERLAY, buf, sizeof(buf)));
  TEST_ASSERT_TRUE(strstr(buf, "fps_overlay") != NULL);
  TEST_ASSERT_TRUE(strstr(buf, "orion_ready") != NULL);
  /* buffer too small */
  TEST_ASSERT_TRUE(!orion_ready_path(ORION_FLAG_FPS_OVERLAY, buf, 8));
  TEST_ASSERT_TRUE(!orion_ready_path(ORION_FLAG_FPS_OVERLAY, NULL, 64));
  return 0;
}

static int test_runtime_flag_fps_overlay(void) {
  orion_ready_clear(ORION_FLAG_FPS_OVERLAY);
  TEST_ASSERT_TRUE(!orion_ready_is_set(ORION_FLAG_FPS_OVERLAY));
  TEST_ASSERT_TRUE(orion_ready_signal(ORION_FLAG_FPS_OVERLAY));
  TEST_ASSERT_TRUE(orion_ready_is_set(ORION_FLAG_FPS_OVERLAY));
  TEST_ASSERT_TRUE(orion_ready_clear(ORION_FLAG_FPS_OVERLAY));
  TEST_ASSERT_TRUE(!orion_ready_is_set(ORION_FLAG_FPS_OVERLAY));
  return 0;
}
...
static int test_runtime_flag_util_booted(void) {
  ...
  TEST_ASSERT_TRUE(orion_ready_signal(ORION_FLAG_UTIL_BOOTED));
  TEST_ASSERT_TRUE(orion_ready_is_set(ORION_FLAG_UTIL_BOOTED));
  orion_ready_clear(ORION_FLAG_UTIL_BOOTED);
  return 0;
}
...
static int test_toolbox_legacy_alias(void) {
  ...
  TEST_ASSERT_TRUE(!orion_ready_is_set(ORION_READY_TOOLBOX));
  TEST_ASSERT_TRUE(orion_ready_signal(ORION_READY_TOOLBOX));
  TEST_ASSERT_TRUE(orion_ready_is_set(ORION_READY_TOOLBOX));
...
int test_ready_suite(void) {
  ...
  failures += orion_test_run("ready_path_builder", test_ready_path_builder);
  failures += orion_test_run("flag_fps_overlay", test_runtime_flag_fps_overlay);
  failures += orion_test_run("flag_util_booted", test_runtime_flag_util_booted);
  failures += orion_test_run("toolbox_legacy_alias", test_toolbox_legacy_alias);
  return failures;
}
