...
uint64_t GetManifestResourceStream_Hook(uint64_t inst, MonoString* FileName) {
    ...
    const bool shortcut = cheats_shortcut_activated || cheats_shortcut_activated_not_open;
    const bool shortcut_not_open = cheats_shortcut_activated_not_open;
    ...
    toolbox::RouteResult route = toolbox::resolve_resource({
        ...
        .cheats_shortcut = cheats_shortcut_activated,
        .cheats_shortcut_not_open = cheats_shortcut_activated_not_open,
    });

    // Publish flags for other hooks (OnPreCreate / OnPress context).
    is_plugin = route.flags.is_plugin;
    is_su_menu = route.flags.is_su_menu;
    is_debug_settings = route.flags.is_debug_settings;
    is_cheats = route.flags.is_cheats;
    is_auto_plugin = route.flags.is_auto_plugin;
    is_remote_play = route.flags.is_remote_play;
    is_plapps = route.flags.is_plapps;
    ...
    case toolbox::Page::Cheats:
        generate_cheats_xml(new_xml_string, current_menu_tid, shortcut,
                            shortcut_not_open);
