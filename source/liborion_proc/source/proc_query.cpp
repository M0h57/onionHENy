 ...
 *
 * sysctl-based process lookup (single implementation for all OrionHEN bins).
 */
...
orion_get_process_name_fn g_get_name = nullptr;
orion_get_app_info_fn g_get_app_info = nullptr;
orion_get_bigapp_id_fn g_get_bigapp = nullptr;
...
extern "C" void orion_proc_set_sce_hooks(orion_get_process_name_fn get_name,
                                         orion_get_app_info_fn get_app_info,
                                         orion_get_bigapp_id_fn get_bigapp) {
  g_get_name = get_name;
  g_get_app_info = get_app_info;
  g_get_bigapp = get_bigapp;
}

extern "C" pid_t orion_find_pid(const char *name) {
  return scan_kinfo(name, /*substr=*/false);
}

extern "C" pid_t find_pid(const char *name) {
  return orion_find_pid(name);
}

extern "C" pid_t orion_find_pid_substr(const char *substr) {
  return scan_kinfo(substr, /*substr=*/true);
}

extern "C" bool orion_proc_is_alive(pid_t pid) {
  if (pid <= 0) {
    return false;
  }
...
extern "C" pid_t orion_find_pid_ex(const char *name, bool needle, bool for_bigapp,
                                   bool need_eboot) {
  if (!name) {
    return -1;
  }
  ...
  if (!for_bigapp) {
    return needle ? orion_find_pid_substr(name) : orion_find_pid(name);
  }
