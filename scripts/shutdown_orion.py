...
Usage:
  python3 scripts/shutdown_orion.py 192.168.x.x
"""
...
ORION_CTRL_TCP_PORT = 9048
ORION_CTRL_TCP_MAGIC = 0x4F52494F  # 'ORIO'
ORION_CTRL_TCP_CMD_SHUTDOWN = 1


def shutdown(host: str, port: int = ORION_CTRL_TCP_PORT, timeout: float = 5.0) -> int:
    frame = struct.pack("<II", ORION_CTRL_TCP_MAGIC, ORION_CTRL_TCP_CMD_SHUTDOWN)
    try:
        ...
        print(
            "Check: PS5 and PC on same LAN; OrionHEN daemon running; port 9048 open.",
            file=sys.stderr,
        )
        return 2
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Shut down OrionHEN stack on PS5")
    ap.add_argument("ps5_ip", help="PS5 LAN IP address")
    ap.add_argument("--port", type=int, default=ORION_CTRL_TCP_PORT)
    args = ap.parse_args()
