...
目标是把 dormant `Fps` 放到 `Search` 后面作为 OnionHEN 入口。
...
patterns = {
    ...
    "new": bytes.fromhex("54 4d 1c e3 1b 4e 15 85 16"),
}
...
### 6. 写 Fps body replacement

`Fps` 原函数会按 `showFps` 决定是否显示。OnionHEN 替换它的前缀，使它
直接渲染一个 icon button：
...
replacement 必须和 old prefix 等长。目前各 profile 使用 77 bytes 前缀。
新增时保留 `static_assert(sizeof(old_prefix) == sizeof(new_body))`。
...
```sh
rg -n "Array: \\['Search', 'Fps', 'Settings', 'Profile'\\]" .tmp/hbc/new_fw_patched.dis
...
这个脚本既是提取工具，也是回归验证工具。新固件适配完成后，脚本应该能在
不带 `--allow-unsupported` 的情况下返回 PASS。
...
```sh
python3 scripts/analyze_rnps_dump.py /path/to/new/DUMP
