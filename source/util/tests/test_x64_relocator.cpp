...
bool relocate(const uint8_t *source, size_t minimum, uintptr_t source_address,
  ...
  std::memset(destination, 0xcc, ORION_X64_TRAMPOLINE_CAPACITY);
  return orion_x64_relocate(
      source, source_address, destination, destination_address, minimum,
      ORION_X64_TRAMPOLINE_CAPACITY, &result);
}

int test_plain_copy_and_jump_back() {
  const uint8_t source[14] = {0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90,
                              0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90};
  uint8_t destination[ORION_X64_TRAMPOLINE_CAPACITY];
  orion_x64_relocate_result result{};
...
int test_rip_relative_out_of_range_fails() {
  ...
  TEST_ASSERT_EQ_INT(ORION_X64_RELOCATE_RIP_DISPLACEMENT_OUT_OF_RANGE,
                     result.error);
  return 0;
}
...
int test_loop_relative_rejected() {
  ...
  TEST_ASSERT_TRUE(!relocate(source, sizeof(source), 0x10000000, 0x11000000,
                             destination, result));
  TEST_ASSERT_EQ_INT(ORION_X64_RELOCATE_UNSUPPORTED_RELATIVE, result.error);
  return 0;
}
...
extern "C" int test_x64_relocator_suite(void) {
  int failures = 0;
  failures += orion_test_run("x64reloc.plain", test_plain_copy_and_jump_back);
  failures += orion_test_run("x64reloc.rip_memory", test_rip_relative_memory);
  failures += orion_test_run("x64reloc.rip_indirect_call",
                             test_rip_relative_indirect_call);
  failures += orion_test_run("x64reloc.call_rel32", test_call_rel32_external);
  failures += orion_test_run("x64reloc.jmp_rel8", test_jmp_rel8_external);
  failures += orion_test_run("x64reloc.jmp_rel32", test_jmp_rel32_external);
  failures += orion_test_run("x64reloc.jcc_internal", test_jcc_short_internal_target);
  failures += orion_test_run("x64reloc.jcc_near", test_jcc_near_external);
  failures += orion_test_run("x64reloc.rip_range", test_rip_relative_out_of_range_fails);
  failures += orion_test_run("x64reloc.loop_reject", test_loop_relative_rejected);
  return failures;
}
