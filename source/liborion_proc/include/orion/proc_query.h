 ...
 * bootstrapper. Complements the kernel allproc helpers in orion/proc.h.
 */
...
bool orion_proc_is_alive(pid_t pid);

typedef int (*orion_get_process_name_fn)(int pid, char *name);
typedef int (*orion_get_app_info_fn)(pid_t pid, void *app_info /* app_info_t */);
typedef int (*orion_get_bigapp_id_fn)(void);

void orion_proc_set_sce_hooks(orion_get_process_name_fn get_name,
...
inline bool isProcessAlive(int pid) noexcept {
  return orion_proc_is_alive(static_cast<pid_t>(pid));
}
#else
static inline bool isProcessAlive(pid_t pid) { return orion_proc_is_alive(pid); }
#endif
