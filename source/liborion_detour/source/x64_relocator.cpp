...
void set_error(orion_x64_relocate_result *result,
               orion_x64_relocate_error error, size_t offset) {
  result->error = error;
...
bool relocate_rip_memory(const RelocatedInstruction &instruction,
  ...
  if (!map_internal_target(target, source_address, source_size,
    ...
    set_error(result, ORION_X64_RELOCATE_INTERNAL_TARGET_NOT_BOUNDARY,
              instruction.source_offset);
    return false;
  }
  ...
  if (new_displacement < INT32_MIN || new_displacement > INT32_MAX) {
    set_error(result, ORION_X64_RELOCATE_RIP_DISPLACEMENT_OUT_OF_RANGE,
              instruction.source_offset);
    return false;
  }
  ...
  *result = {};
  ...
  while (source_size < min_source_size) {
    if (instruction_count >= kMaxInstructions) {
      set_error(result, ORION_X64_RELOCATE_TOO_MANY_INSTRUCTIONS, source_size);
      return false;
    }
    ...
    if (decoded_size == 0 || (instruction.decoded.flags & F_ERROR)) {
      set_error(result, ORION_X64_RELOCATE_DECODE_ERROR, source_size);
      return false;
    }
    instruction.destination_size = instruction.decoded.len;
    if (!classify_relative(instruction)) {
      set_error(result, ORION_X64_RELOCATE_UNSUPPORTED_RELATIVE, source_size);
      return false;
    }
  ...
  if (destination_size > destination_capacity ||
    ...
    set_error(result, ORION_X64_RELOCATE_OUTPUT_TOO_SMALL, source_size);
    return false;
  }
  ...
  for (size_t i = 0; i < instruction_count; ++i) {
    ...
    if (!relative_target(instruction, source_address, &target)) {
      set_error(result, ORION_X64_RELOCATE_UNSUPPORTED_RELATIVE,
                instruction.source_offset);
      return false;
    }
    if (!map_internal_target(target, source_address, source_size,
  ...
  result->error = ORION_X64_RELOCATE_OK;
  result->error_offset = 0;
  return true;
}

extern "C" const char *orion_x64_relocate_error_string(
    orion_x64_relocate_error error) {
  switch (error) {
  case ORION_X64_RELOCATE_OK:
    return "ok";
  case ORION_X64_RELOCATE_INVALID_ARGUMENT:
    return "invalid argument";
  case ORION_X64_RELOCATE_DECODE_ERROR:
    return "instruction decode error";
  case ORION_X64_RELOCATE_TOO_MANY_INSTRUCTIONS:
    return "too many instructions";
  case ORION_X64_RELOCATE_OUTPUT_TOO_SMALL:
    return "trampoline output too small";
  case ORION_X64_RELOCATE_UNSUPPORTED_RELATIVE:
    return "unsupported relative instruction";
  case ORION_X64_RELOCATE_INTERNAL_TARGET_NOT_BOUNDARY:
    return "internal target is not an instruction boundary";
  case ORION_X64_RELOCATE_RIP_DISPLACEMENT_OUT_OF_RANGE:
